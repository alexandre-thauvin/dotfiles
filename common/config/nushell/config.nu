# env vars
$env.M2_HOME = '/Users/athauvin/work/apache-maven-3.9.7'
$env.ANDROID_HOME = '/Applications/Android Studio.app/Contents'
#$env.JAVA_HOME = exec /usr/libexec/java_home -v 17 | str trim


# PATH
use std/util "path add"
path add "/opt/homebrew/bin" $"($env.ANDROID_HOME)/tools" $"($env.ANDROID_HOME)/tools/bin" $"($env.ANDROID_HOME)/platform-tools" $"($env.ANDROID_HOME)/emulator" "/opt" $"($env.M2_HOME)" "/Users/athauvin/scripts" "/opt/android-studio/bin" "/usr/local/share/dotnet" "/usr/local/share"
